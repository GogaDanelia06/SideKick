    ...
    <div className="mt-4 flex flex-col gap-3">
      <div className="flex items-center gap-2 rounded-[8px] border border-border2 bg-soft py-1.5 pl-3 pr-1.5 text-[13px]">
        ...
        <span className="min-w-0 flex-1 truncate text-muted">
          <span className="text-ink">{file.name}</span> · {count} {t("dashboard.products.chosenFileCard.products")}
          {file.errors.length > 0 ? (
            <span className="text-red"> · {file.errors.length} {t("dashboard.products.chosenFileCard.errorS")}</span>
          ) : null}
        </span>
        <button
          type="button"
          onClick={onRemove}
          disabled={importing}
          title={t("dashboard.products.chosenFileCard.removeFile")}
          aria-label={t("dashboard.products.chosenFileCard.removeFile")}
      ...
      {file.errors.length > 0 ? (
        <div className="rounded-[8px] border border-red/40 bg-red-surface p-3 text-[13px]">
          <p className="mb-1.5 font-medium text-red">
            {t("dashboard.products.chosenFileCard.fixTheseRowsIn")}
          </p>
          <ul className="flex flex-col gap-0.5 text-ink">
            ...
            {hidden > 0 ? <li className="text-muted">{t("dashboard.products.chosenFileCard.andMore", { count: hidden })}</li> : null}
          </ul>
        </div>
      ) : done ? null : (
        <button
        ...
        >
          <IconUpload size={16} />
          {importing
            ? t("dashboard.products.chosenFileCard.importing")
            : t("dashboard.products.chosenFileCard.importCount", { count: file.rows.length })}
        </button>
      )}
