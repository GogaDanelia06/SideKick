...
const HEADING = "mb-1 text-[11px] font-semibold uppercase tracking-wide text-muted";
...
export function ThemePreview({ colors, shade }: { colors: ThemeColors; shade: Shade }) {
  ...
  return (
    <div className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card">
      <header className="shrink-0 border-b border-border2 px-3.5 py-3">
        ...
        <p className="mt-1 text-[12px] leading-snug text-muted">
          {t({
            ka: "მიიტანე მაუსი ფერზე — აქ მოინიშნება.",
            en: "Point at a colour to see where it is used.",
          })}
        </p>
      </header>

      <div className="flex min-h-0 flex-col gap-2.5 overflow-y-auto p-2.5">
        <div>
